...
public class OrderController(ApplicationDbContext context) : Controller
{
    ...
    public IActionResult SubmitCheckout(int selectedAddressId, Enums.OrderType selectedType, decimal? CustomTipAmount, string TipAmount, double deliveryDistanceKm = 0)
    {
        ...
        UserAddress? selectedAddress = _context.UserAddresses.FirstOrDefault(a => a.Id == selectedAddressId);
        if (selectedAddress != null)
        {
            // Optional: process address if needed
        }
    ...
    private void SaveOrderAndUserPoints(Order cartOrder, decimal total, decimal tipAmount, Enums.OrderType selectedType, int userId)
    {
        User? user = _context.Users.Include(u => u.Orders).FirstOrDefault(u => u.Id == userId);
        if (user == null) return;

        cartOrder.Subtotal = total - tipAmount;
        cartOrder.TipAmount = tipAmount;
        cartOrder.Total = total;
        cartOrder.Status = Enums.OrderStatus.Pending;
